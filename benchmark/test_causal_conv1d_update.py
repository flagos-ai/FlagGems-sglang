...
BENCH_IDS = ["batch64_dim4096", "batch256_dim2048"]
...
def test_causal_conv1d_update_perf(case_idx):
    ...
    try:
        causal_conv1d_update(**kwargs)
    except NotImplementedError:
        pytest.skip("mamba/causal_conv1d_update not yet implemented")
        return

    ref_us = do_bench_us(lambda: reference(**kwargs))
    triton_us = do_bench_us(lambda: causal_conv1d_update(**kwargs))

    record_case(
        "mamba/causal_conv1d_update", BENCH_IDS[case_idx], ref_us, triton_us
    )
