...
def test_chunk_state_varlen_perf(case_idx):
    ...
    try:
        chunk_state_varlen(**kwargs)
    except NotImplementedError:
        pytest.skip("mamba/chunk_state_varlen not yet implemented")
        return

    ref_us = do_bench_us(lambda: reference(**kwargs))
    triton_us = do_bench_us(lambda: chunk_state_varlen(**kwargs))
    record_case(
        "mamba/chunk_state_varlen", f"case{case_idx}", ref_us, triton_us
    )
