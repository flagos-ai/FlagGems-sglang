...
def test_fused_moe_router_cudacore_perf(case_idx):
    ...
    try:
        fused_moe_router_cudacore(**kwargs)
    except NotImplementedError:
        pytest.skip("moe/fused_moe_router_cudacore not yet implemented")
        return

    ref_us = do_bench_us(lambda: reference(**kwargs))
    triton_us = do_bench_us(lambda: fused_moe_router_cudacore(**kwargs))

    record_case(
        "moe/fused_moe_router_cudacore", BENCH_IDS[case_idx], ref_us, triton_us
    )
