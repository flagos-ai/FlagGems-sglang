...
def test_fused_rmsnorm_perf(case_idx):
    ...
    try:
        fused_rmsnorm(**kwargs)
    except NotImplementedError:
        pytest.skip("activation_norm/fused_rmsnorm not yet implemented")
        return

    ref_us = do_bench_us(lambda: reference(**kwargs))
    triton_us = do_bench_us(lambda: fused_rmsnorm(**kwargs))
    record_case(
        "activation_norm/fused_rmsnorm", f"case{case_idx}", ref_us, triton_us
    )
