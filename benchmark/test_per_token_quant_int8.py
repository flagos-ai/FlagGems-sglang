...
# ---------------------------------------------------------------------------
# Cases (from kernel-comp-baseline/problems/quantization/per_token_quant_int8/cases.py)
# ---------------------------------------------------------------------------


# Inlined from per_token_group_quant_int8 (decoupled from problems repo)
...
def test_per_token_quant_int8_perf(case_idx):
    ...
    try:
        from flaggems_sglang.ops.per_token_quant_int8 import (
            per_token_quant_int8,
        )
    except (ImportError, ModuleNotFoundError):
        pytest.skip("quantization/per_token_quant_int8 ops module not found")
        return

    try:
        per_token_quant_int8(**kwargs)
    except NotImplementedError:
        pytest.skip("quantization/per_token_quant_int8 not yet implemented")
        return

    ref_us = do_bench_us(lambda: reference(**kwargs))
    triton_us = do_bench_us(lambda: per_token_quant_int8(**kwargs))

    record_case(
        "quantization/per_token_quant_int8",
        BENCH_IDS[case_idx],
        ref_us,
        triton_us,
    )
