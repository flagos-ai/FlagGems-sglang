...
def tune_s6_m64n32k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 6, 64, 32, 64)


def tune_s10_m64n32k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 10, 64, 32, 64)
...
def tune_s6_m32n32k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 6, 32, 32, 64)


def tune_s10_m32n32k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 10, 32, 32, 64)
...
def tune_s8_m32n64k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 8, 32, 64, 64)
...
def chunked_sgmv_shrink(x, weights, batch_info, num_slices=1):
    ...
    if batch_info.permutation is not None:
        if x.shape[1] >= 1024:
            if (
                batch_info.max_len <= 8
                and weights.shape[1] <= 16
                and torch.version.hip is None
            ):
                return _launch_split(
                    x, weights, batch_info, stack_num, 16, 16, 16, 64
                )
            if torch.version.hip is None:
                return _launch_split(
                    x, weights, batch_info, stack_num, 10, 32, 32, 64
                )
            return _launch_split(
                x, weights, batch_info, stack_num, 16, 64, 32, 64
            )
        return _launch_split(x, weights, batch_info, stack_num, 1, 64, 32, 64)
    if x.shape[1] >= 1024:
        if torch.version.hip is None:
