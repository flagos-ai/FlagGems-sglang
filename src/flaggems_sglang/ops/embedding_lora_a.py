...
def _embedding_lora_a_kernel(
    ...
    rank_offsets = rank_block * BLOCK_R + tl.arange(0, BLOCK_R)
    rows = start + token_offsets
    row_mask = rows < end
    ...
    clamped_tokens = tl.minimum(tokens, vocab_size - 1)
    weight_offsets = (
        weight_index * RANK + rank_offsets[None, :]
    ) * vocab_size + clamped_tokens[:, None]
    values = tl.load(weights + weight_offsets, mask=valid, other=0.0)
    if HAS_EXTRA:
    ...
    values = tl.where(rank_offsets[None, :] < active_rank, values, 0.0)

    output_offsets = rows[:, None] * RANK + rank_offsets[None, :]
    tl.store(
        output + output_offsets,
        values,
        mask=row_mask[:, None] & (rank_offsets[None, :] < RANK),
    )
...
def embedding_lora_a(
    ...
    _embedding_lora_a_kernel[(batch_info.bs * token_blocks * rank_blocks,)](
        input_ids,
        weights,
