...
def _replicate(X, W, B, Y, META: tl.constexpr, SPLIT: tl.constexpr):
    ...
    K = D * S
    i = tl.arange(0, T)
    shift = tl.load(X + row * K, cache_modifier=".cg").to(tl.float32)
    x = tl.load(X + row * K + i, i < K, 0, cache_modifier=".cg").to(tl.float32)
    ...
    z = (
        tl.load(X + row * K + j, valid, 0, cache_modifier=".cg").to(tl.float32)
        - shift
    )
    scale = r * w
