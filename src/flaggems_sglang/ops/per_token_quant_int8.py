...
def _per_token_quant_int8(
    ...
    if N == BLOCK_N:
        x = tl.load(x_ptr + row * N + offs, cache_modifier=".cg").to(
            tl.float32
        )
    else:
        x = tl.load(
    ...
    if N == BLOCK_N:
        tl.store(q_ptr + row * N + offs, q, cache_modifier=".cs")
    else:
        tl.store(
            q_ptr + row * N + offs, q, mask=offs < N, cache_modifier=".cs"
        )
    tl.store(s_ptr + row, scale, cache_modifier=".cs")
