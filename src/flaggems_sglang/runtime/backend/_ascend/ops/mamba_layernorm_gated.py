...
def _performance_kernel(
    ...
    for task in tl.range(program, task_count, program_count):
        ...
        if use_fp16_affine:
            weight = tl.load(weight_ptr + feature_offsets).to(tl.float16)
            bias = tl.load(bias_ptr + feature_offsets).to(tl.float16)
            affine = (
                normalized.to(tl.float16) * weight[None, :] + bias[None, :]
            )
...
def mamba_layernorm_gated(
    ...
    if (
        ...
        if x.dtype == torch.bfloat16 and row_count == 1:
            rows_per_program, num_warps, grid_cap, num_stages = 32, 1, 256, 2
        elif x.dtype == torch.bfloat16 and row_count == 8:
            rows_per_program, num_warps, grid_cap, num_stages = 32, 1, 1024, 2
        elif x.dtype == torch.bfloat16 and row_count == 64:
            rows_per_program, num_warps, grid_cap, num_stages = 8, 1, 512, 2
        elif x.dtype == torch.bfloat16 and row_count == 512:
            rows_per_program, num_warps, grid_cap, num_stages = 16, 1, 256, 2
        elif x.dtype == torch.bfloat16:
            rows_per_program, num_warps, grid_cap, num_stages = 64, 1, 512, 1
        elif row_count == 1:
            rows_per_program, num_warps, grid_cap, num_stages = 8, 4, 512, 2
        elif row_count == 8:
            rows_per_program, num_warps, grid_cap, num_stages = 4, 4, 512, 2
        elif row_count == 64:
            rows_per_program, num_warps, grid_cap, num_stages = 8, 4, 512, 2
        else:
            rows_per_program, num_warps, grid_cap, num_stages = 8, 1, 512, 2
        ...
        _performance_kernel[(min(task_count, grid_cap),)](
            ...
            group_size,
            rows_per_program,
            x.dtype == torch.bfloat16 and row_count in (1, 4096),
            num_warps=num_warps,
