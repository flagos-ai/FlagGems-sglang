...
def _moe_fused_gate_rows_kernel(
    ...
    if NUM_GROUPS > 1:
        ...
        group_of_expert = expert_offsets // EXPERTS_PER_GROUP
        group_scores = tl.full(
            (BLOCK_M, BLOCK_G), -float("inf"), dtype=tl.float32
        )
        for group in tl.static_range(0, NUM_GROUPS):
            in_group = (group_of_expert[None, :] == group) & valid_experts[
                None, :
            ]
            group_values = tl.where(in_group, selection, -float("inf"))
            first_value = tl.max(group_values, axis=1)[:, None]
            first_expert = tl.max(
                tl.where(
            ...
            second_value = tl.max(
                tl.where(
                    expert_offsets[None, :] != first_expert,
                    group_values,
                    -float("inf"),
                ),
                axis=1,
            ...
            group_scores = tl.where(
                group_offsets[None, :] == group,
                first_value + second_value,
                group_scores,
            )

        kept_experts = tl.zeros((BLOCK_M, BLOCK_N), dtype=tl.int1)
        remaining_groups = group_scores
        for _ in tl.static_range(0, TOPK_GROUP):
            best_group_score = tl.max(remaining_groups, axis=1)[:, None]
            best_group = tl.max(
                tl.where(
            ...
            kept_experts = kept_experts | (
                group_of_expert[None, :] == best_group
            )
    ...
    for rank in tl.static_range(0, K_ROUTED):
        ...
        if USE_OUTPUT_SCRATCH:
            rank_output = row_offsets * K + rank
            tl.store(
                weights_ptr + rank_output,
                tl.reshape(weight, (BLOCK_M,)),
                mask=valid_rows,
            )
            tl.store(
                indices_ptr + rank_output,
                tl.reshape(best_expert, (BLOCK_M,)),
                mask=valid_rows,
            )
        else:
    ...
    if USE_OUTPUT_SCRATCH:
        for shared_rank in tl.static_range(K_ROUTED, K):
            shared_output = row_offsets * K + shared_rank
            tl.store(
                weights_ptr + shared_output,
                tl.reshape(routed_sum / ROUTED_SCALE, (BLOCK_M,)),
                mask=valid_rows,
            )
            tl.store(
                indices_ptr + shared_output,
        ...
        tl.store(weights_ptr + output_offsets, values, mask=output_mask)
        tl.store(
            indices_ptr + output_offsets, output_indices, mask=output_mask
        )
...
def moe_fused_gate(
    ...
    max_grid_x = 32768
    if M <= max_grid_x:
        use_rows = M >= 2 and num_expert_group == 1 and block_n <= 256
        if use_rows:
            if block_n == 128:
                block_m = 32
            elif block_n == 256:
            ...
            _moe_fused_gate_rows_kernel[(triton.cdiv(M, block_m),)](
                ...
                BLOCK_G=triton.next_power_of_2(int(num_expert_group)),
                USE_OUTPUT_SCRATCH=bool(num_expert_group > 1),
                BLOCK_M=block_m,
                BLOCK_N=block_n,
                BLOCK_K=block_k,
                num_warps=num_warps,
    ...
    for row_start in range(0, M, max_grid_x):
        ...
        use_rows = grid_rows >= 2 and num_expert_group == 1 and block_n <= 256
        if use_rows:
            if block_n == 128:
                block_m = 32
            elif block_n == 256:
