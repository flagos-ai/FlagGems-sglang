...
def _group(
    ...
    cols = tl.cdiv(D, B)
    for job in range(
        tl.program_id(0), tl.cdiv(ROWS, 2) * cols, tl.num_programs(0)
    ):
        tile = job.to(tl.int64 if WIDE else tl.int32)
        row = (tile // cols) * 2
        col = (tile % cols) * B
        rp = tl.make_block_ptr(
            R, (ROWS, D), (D, 1), (row, col), (2, B), (1, 0)
        )
        up = tl.make_block_ptr(
            U, (ROWS, D), (D, 1), (row, col), (2, B), (1, 0)
        )
...
def residual_gate_add(residual, update, gate):
    ...
    if n:
        d = residual.shape[-1]
        rows = n // d
        b = min(1024, triton.next_power_of_2(d))
        _group[(min(triton.cdiv(rows, 2) * triton.cdiv(d, b), 48),)](
            residual,
            update,
            gate,
            out,
