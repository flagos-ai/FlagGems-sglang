...
def _chunked_sgmv_row_kernel(
    ...
    if DIRECT_UNIFORM:
        batch_id = position // MAX_LEN
        active = position < BS * MAX_LEN
    else:
        batch_offsets = tl.arange(0, BLOCK_B)
        valid_batch = batch_offsets < BS
        starts = tl.load(seg_indptr + batch_offsets, mask=valid_batch, other=0)
        ends = tl.load(
            seg_indptr + batch_offsets + 1, mask=valid_batch, other=0
        )
        owns_position = valid_batch & (position >= starts) & (position < ends)
        batch_id = tl.max(tl.where(owns_position, batch_offsets, 0), axis=0)
        active = tl.sum(owns_position.to(tl.int32), axis=0) != 0

    row = tl.load(permutation + position, mask=active, other=0)
    adapter = tl.load(weight_indices + batch_id, mask=active, other=0)
    n_mask = active & (n_offsets < N)
    ...
    for k_start in tl.range(0, K, BLOCK_K):
        ...
        k_mask = k_offsets < K
        x_values = tl.load(
            x + row * stride_xm + k_offsets * stride_xk,
            mask=active & k_mask,
            other=0.0,
        ).to(tl.float32)
        ...
        accumulator += tl.sum(weight_values * x_values[None, :], axis=1)

    tl.store(
        output + row * stride_om + n_offsets * stride_on,
        accumulator,
        mask=n_mask,
    )


def _launch_row_sgmv(x, weights, batch_info, block_k=512, block_n=32):
    output = torch.zeros(
        (x.shape[0], weights.shape[1]), dtype=x.dtype, device=x.device
    )
    block_b = triton.next_power_of_2(max(1, batch_info.bs))
    expected_tokens = getattr(batch_info, "expected_tokens", None)
    direct_uniform = (
        expected_tokens == x.shape[0]
        and x.shape[0] == batch_info.bs * batch_info.max_len
    )
    _chunked_sgmv_row_kernel[
        (x.shape[0], triton.cdiv(weights.shape[1], block_n))
    ](
        x,
        weights,
        ...
        N=weights.shape[1],
        BS=batch_info.bs,
        MAX_LEN=max(1, batch_info.max_len),
    ...
    if gcu_options:
        if gcu_fp_fusion:
            _sgemm_lora_a_kernel[grid](
                *arguments, num_warps=1, num_stages=1, enable_fp_fusion=True
            )
        else:
            _sgemm_lora_a_kernel[grid](*arguments, num_warps=1, num_stages=1)
    else:
        _sgemm_lora_a_kernel[grid](
            *arguments, num_warps=num_warps, num_stages=num_stages
        )
    return output
...
def _launch_split(
    ...
    if gcu_options:
        if gcu_fp_fusion:
            ...
            _sgemm_lora_a_split_kernel[split_grid](
                *split_arguments, num_warps=1, num_stages=1
            )
    else:
        _sgemm_lora_a_split_kernel[split_grid](
            *split_arguments,
    ...
    reduce_block_m = block_m if reduce_block_m is None else reduce_block_m
    reduce_block_n = block_n if reduce_block_n is None else reduce_block_n
    reduce_grid = (
        triton.cdiv(token_count, reduce_block_m),
        triton.cdiv(output_size, reduce_block_n),
    )
    reduce_arguments = (
        partial,
        output,
    ...
    if gcu_options:
        if gcu_fp_fusion:
            ...
            _sgemm_lora_a_reduce_kernel[reduce_grid](
                *reduce_arguments, num_warps=1, num_stages=1
            )
    else:
        _sgemm_lora_a_reduce_kernel[reduce_grid](
            *reduce_arguments,
...
class _PackedBatchInfo:
    pass


def _launch_direct_permuted(x, weights, batch_info, stack_num):
    token_count, hidden_size = x.shape
    output_size = weights.shape[1]
    packed_x = torch.empty_like(x)
    input_elements = token_count * hidden_size
    ...
    packed_info = _PackedBatchInfo()
    packed_info.bs = batch_info.bs
    packed_info.max_len = batch_info.max_len
    packed_info.seg_indptr = batch_info.seg_indptr
    packed_info.weight_indices = batch_info.weight_indices
    packed_info.permutation = None
    if batch_info.max_len <= 64:
        packed_output = _launch(
            packed_x,
            weights,
            packed_info,
            stack_num,
...
def tune_gcu_m16n32k1024(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_gcu_m32n32k512(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_gcu_m32n32k2048(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_gcu_m32n32k4096(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_gcu_m32n32k4096_fused(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_m32n32k4096_w2(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_m32n32k4096_w4(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_m64n32k4096_w1(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_m64n64k4096_w1(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_m128n64k4096_w1(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_m256n64k4096_w1(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_m64n64k2048_w1(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_m32n64k2048_w1(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_m64n64k1024_w1(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_m128n64k1024_w1(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_m128n64k2048_w1(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_m256n64k2048_w1(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_gcu_m64n32k512(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_gcu_m64n32k1024(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_gcu_m64n32k2048(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_gcu_m128n64k512(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_gcu_m256n64k512(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_gcu_s2_m256n64k512(x, weights, batch_info, stack_num=1):
    return _launch_split(
        x,
        weights,
        batch_info,
...
def tune_gcu_s4_m128n64k512(x, weights, batch_info, stack_num=1):
    return _launch_split(
        x,
        weights,
        batch_info,
...
def tune_gcu_s8_m128n64k512(x, weights, batch_info, stack_num=1):
    return _launch_split(
        x,
        weights,
        batch_info,
...
def tune_gcu_s8_m256n64k512(x, weights, batch_info, stack_num=1):
    return _launch_split(
        x,
        weights,
        batch_info,
...
def tune_gcu_s8_m256n64k512_r32(x, weights, batch_info, stack_num=1):
    return _launch_split(
        x, weights, batch_info, stack_num, 8, 256, 64, 512, True, False, 32, 64
    )


def tune_gcu_s8_m256n64k512_r64(x, weights, batch_info, stack_num=1):
    return _launch_split(
        x, weights, batch_info, stack_num, 8, 256, 64, 512, True, False, 64, 64
    )


def tune_gcu_s8_m256n64k512_r256(x, weights, batch_info, stack_num=1):
    return _launch_split(
        x,
        weights,
        batch_info,
...
def tune_gcu_s4_m64n64k512(x, weights, batch_info, stack_num=1):
    return _launch_split(
        x, weights, batch_info, stack_num, 4, 64, 64, 512, True, False, 128, 64
    )


def tune_gcu_s8_m64n64k512(x, weights, batch_info, stack_num=1):
    return _launch_split(
        x, weights, batch_info, stack_num, 8, 64, 64, 512, True, False, 128, 64
    )


def tune_gcu_s8_m256n64k1024(x, weights, batch_info, stack_num=1):
    return _launch_split(
        x,
        weights,
        batch_info,
...
def tune_gcu_s16_m128n64k512(x, weights, batch_info, stack_num=1):
    return _launch_split(
        x,
        weights,
        batch_info,
...
def tune_gcu_s16_m256n64k512(x, weights, batch_info, stack_num=1):
    return _launch_split(
        x,
        weights,
        batch_info,
...
def chunked_sgmv_shrink(x, weights, batch_info, num_slices=1):
    ...
    if x.dtype == torch.float32:
        return _launch(x, weights, batch_info, stack_num, 16, 32, 32, 4, 2)
    if batch_info.permutation is not None and x.shape[1] >= 1024:
        return _launch_row_sgmv(
            x,
            weights,
            batch_info,
            block_k=2048 if x.shape[1] >= 4096 else 1024,
            block_n=(
                64
                if weights.shape[1] >= 64
                else 16 if weights.shape[1] <= 16 else 32
            ),
        )
