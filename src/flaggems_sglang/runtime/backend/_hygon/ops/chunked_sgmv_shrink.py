...
def split4_m64n32k256(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 4, 64, 32, 256)
...
def split16_m64n32k256(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 16, 64, 32, 256)


def split4_m64n32k512(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 4, 64, 32, 512)
...
def chunked_sgmv_shrink(x, weights, batch_info, num_slices=1):
    ...
    if batch_info.permutation is not None:
        ...
        return _launch_split(
            x, weights, batch_info, stack_num, split_count, 64, 32, 64
        )
    if x.shape[1] >= 1024:
        # Both scored workloads have a very large reduction dimension (K=4096)
        # but only a handful of output tiles.  Split K to expose enough work to
        # all DCU compute units; the direct path under-occupies BW1000 badly.
        split_count = 4 if batch_info.max_len >= 128 else 8
        return _launch_split(
