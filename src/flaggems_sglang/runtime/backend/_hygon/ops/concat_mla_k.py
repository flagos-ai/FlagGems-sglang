...
def _heads(
    ...
    if F == 2:
        A = A.to(tl.pointer_type(tl.uint32))
        R = R.to(tl.pointer_type(tl.uint32))
        OUT = OUT.to(tl.pointer_type(tl.uint32))
    elif F == 4:
    ...
    for task in range(tl.program_id(0), T * groups, NP):
        ...
        if N > 0:
            ...
            tl.store(
                OUT + (token * H + heads[:, None]) * (N + D) + a[None, :],
                v,
                (heads[:, None] < H) & (a[None, :] < N),
            )
        if D > 0:
            ...
            tl.store(
                OUT + (token * H + heads[:, None]) * (N + D) + N + r[None, :],
