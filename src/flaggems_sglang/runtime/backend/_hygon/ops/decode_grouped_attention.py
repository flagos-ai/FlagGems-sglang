...
def _flash128_static_kernel(
    ...
    batch_id = tl.program_id(0)
    head_offsets = tl.arange(0, H_Q)
    dim_offsets = tl.arange(0, D)
    value_offsets = tl.arange(0, D_V)
    query_values = tl.load(
        q + (batch_id * H_Q + head_offsets[:, None]) * D + dim_offsets[None, :]
    )
    running_max = tl.full((H_Q,), -float("inf"), tl.float32)
    running_sum = tl.zeros((H_Q,), tl.float32)
    accumulator = tl.zeros((H_Q, D_V), tl.float32)
    sequence_start = tl.load(kv_indptr + batch_id)
    for token_start in tl.range(0, SEQUENCE_LENGTH, BLOCK_N):
        token_offsets = token_start + tl.arange(0, BLOCK_N)
        pages = tl.load(kv_indices + sequence_start + token_offsets)
        key_values = tl.load(
            k_buffer + pages[:, None] * D + dim_offsets[None, :]
        )
        scores = tl.dot(query_values, tl.trans(key_values)) * sm_scale
        block_max = tl.max(scores, axis=1)
        next_max = tl.maximum(running_max, block_max)
        old_scale = tl.exp2((running_max - next_max) * 1.4426950408889634)
        probabilities = tl.exp2(
            (scores - next_max[:, None]) * 1.4426950408889634
        )
        value_values = tl.load(
            v_buffer + pages[:, None] * D_V + value_offsets[None, :]
        )
        accumulator = accumulator * old_scale[:, None] + tl.dot(
    ...
    output_offsets = (
        batch_id * H_Q + head_offsets[:, None]
    ) * D_V + value_offsets[None, :]
    tl.store(output + output_offsets, accumulator / running_sum[:, None])
...
def decode_grouped_attention(
    ...
    if specialized_path and batch_size <= 8:
        block_h = 32
        block_n = 32
        block_d = 32
    ...
    elif specialized_path:
        _flash128_static_kernel[(batch_size,)](
            q,
            k_buffer,
            v_buffer,
            kv_indptr,
