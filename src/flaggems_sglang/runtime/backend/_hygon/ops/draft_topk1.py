...
_PARTITION_SIZE = 262144
_INT64_STORAGE_IS_INT32 = False
_INT64_DRAFT_IS_INT32_PAIRS = False
_ARGMAX_VIA_MAX_MIN = False
...
def _write_outputs(
    ...
    if HAS_DRAFT_TOKENS:
        ...
        if INT64_DRAFT_IS_INT32_PAIRS:
            safe_columns = tl.minimum(columns, DRAFT_SIZE - 1)
            input_offsets = (
                row * stride_draft_batch + safe_columns * stride_draft_column
            )
            output_offsets = (
                row * stride_out_draft_batch
                + columns * stride_out_draft_column
            )
            low_words = tl.load(draft_tokens_ptr + input_offsets)
            high_words = tl.load(draft_tokens_ptr + input_offsets + 1)
            is_selected = columns == DRAFT_TOKEN_COLUMN
            low_words = tl.where(is_selected, selected_index, low_words)
            high_words = tl.where(is_selected, 0, high_words)
            tl.store(
                out_draft_tokens_ptr + output_offsets,
                low_words,
                mask=mask,
            )
            tl.store(
                out_draft_tokens_ptr + output_offsets + 1,
                high_words,
                mask=mask,
            )
        else:
...
def _draft_topk1_direct_kernel(
    ...
    HAS_DRAFT_TOKENS: tl.constexpr,
    INT64_DRAFT_IS_INT32_PAIRS: tl.constexpr,
    ARGMAX_VIA_MAX_MIN: tl.constexpr,
):
    row = tl.program_id(0)
...
def _draft_topk1_partial_kernel(
    ...
    if ARGMAX_VIA_MAX_MIN:
        maximum = tl.max(logits, axis=0)
        local_columns = tl.arange(0, BLOCK_VOCAB)
        candidates = tl.where(
            logits == maximum,
            local_columns,
            BLOCK_VOCAB,
        )
        local_index = tl.min(candidates, axis=0)
    else:
...
def _draft_topk1_finalize_kernel(
    ...
    if ARGMAX_VIA_MAX_MIN:
        maximum = tl.max(partial_values, axis=0)
        candidates = tl.where(
            partial_values == maximum,
            partitions,
            BLOCK_PARTITIONS,
        )
        selected_partition = tl.min(candidates, axis=0)
    else:
...
def chunk_topk1_outputs(
    ...
    if has_draft_tokens:
        ...
        elif _INT64_DRAFT_IS_INT32_PAIRS and draft_tokens.dtype == torch.int64:
            draft_tokens_ptr = draft_tokens.view(torch.int32).reshape(-1)
            out_draft_tokens_ptr = out_draft_tokens.view(torch.int32).reshape(
                -1
            )
    ...
    output_meta = {
        ...
        "ARGMAX_VIA_MAX_MIN": _ARGMAX_VIA_MAX_MIN,
    }
    return (
        topk_p,
        topk_index,
