...
def mamba_layernorm_gated(
    ...
    if (
        ...
        if x.dtype == torch.bfloat16:
            if row_count == 1:
                rows_per_program, num_warps, row_first = 32, 4, False
            elif row_count == 8:
                rows_per_program, num_warps, row_first = 16, 8, True
            elif row_count == 64:
                rows_per_program, num_warps, row_first = 32, 4, False
            elif row_count == 512:
                rows_per_program, num_warps, row_first = 32, 4, True
            else:
                rows_per_program, num_warps, row_first = 32, 4, False
            row_blocks = triton.cdiv(row_count, rows_per_program)
            grid = (
                (row_blocks, group_count)
                if row_first
                else (group_count, row_blocks)
            )
            kernel = (
            ...
            return output
        if row_count == 4096:
            rows_per_program, num_warps = 8, 1
        else:
