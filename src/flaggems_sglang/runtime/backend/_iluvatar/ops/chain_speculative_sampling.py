...
def base_sum_kernel(
    ...
    value = a0 + a1 + a2 + a3
    for j in tl.static_range(WARP_LOG, LOG_NT):
        offset = NT >> j - WARP_LOG + 1
...
def base_plain(
    ...
    predicts_i = predicts
    accept_i = accept_index
    last_slot = torch.empty(batch, dtype=index.dtype, device=source.device)
...
def _soft64_plain(
    ...
    sums = torch.empty((batch,), dtype=target.dtype, device=target.device)
    nt = min(
        1024, 1 << (vocab // 4 if vocab > 128 else vocab).bit_length() - 1
    )
    base_values_kernel[batch, triton.cdiv(vocab, 256)](
        values,
        target,
    ...
    base_sum_kernel[batch,](
        ...
        nt,
        nt.bit_length() - 1,
        min(6, nt.bit_length() - 1),
        enable_fp_fusion=False,
    )
    _soft64_sample[batch,](
        values,
        sums,
        coins_final,
        predicts_i,
...
def _decode32(bits):
    ...
    if exponent == -127:
        exponent = -126
        while (mantissa != 0) & (mantissa < 8388608):
            mantissa = mantissa << 1
            exponent -= 1
    else:
...
@triton.jit
def _shift_sticky(high, low, distance):
    result_high = high
    result_low = low
    if distance != 0:
        if distance < 32:
            sticky = low & (tl.full((), 1, tl.uint32) << distance) - 1 != 0
            result_high = high >> distance
            result_low = (
                low >> distance | high << 32 - distance | sticky.to(tl.uint32)
            )
        elif distance < 64:
            count = distance - 32
            sticky = (low != 0) | (
                high & (tl.full((), 1, tl.uint32) << count) - 1 != 0
            )
            result_high = tl.full((), 0, tl.uint32)
            result_low = high >> count | sticky.to(tl.uint32)
        else:
            result_high = tl.full((), 0, tl.uint32)
            result_low = ((high != 0) | (low != 0)).to(tl.uint32)
    return (result_high, result_low)


@triton.jit
def _add32_to64(exponent, high, low, bits):
    incoming_exp, incoming_high = _decode32(bits)
    incoming_low = tl.full((), 0, tl.uint32)
    if incoming_high != 0:
        if high == 0:
            exponent = incoming_exp
            high = incoming_high
            low = incoming_low
        else:
            if incoming_exp > exponent:
                exponent, incoming_exp = (incoming_exp, exponent)
                high, incoming_high = (incoming_high, high)
                low, incoming_low = (incoming_low, low)
            small_high, small_low = _shift_sticky(
                incoming_high, incoming_low, exponent - incoming_exp
            )
            combined_low = low + small_low
            high = high + small_high + (combined_low < low).to(tl.uint32)
            low = combined_low
            if high >= 16777216:
                high, low = _shift_sticky(high, low, 1)
                exponent += 1
            remainder = low & 7
            round_up = (remainder > 4) | (remainder == 4) & (low & 8 != 0)
            previous = low & 4294967288
            low = previous + tl.where(round_up, 8, 0).to(tl.uint32)
            high += (low < previous).to(tl.uint32)
            if high >= 16777216:
                high, low = _shift_sticky(high, low, 1)
                exponent += 1
    return (exponent, high, low)


@triton.jit
def _soft64_sample(Values, Sums, Coins, Predicts, Slots, V: tl.constexpr):
    row = tl.program_id(0)
    threshold = tl.load(Sums + row).to(tl.float32) * tl.load(Coins + row).to(
        tl.float32
    )
    target_exp, target_high = _decode32(threshold.to(tl.uint32, bitcast=True))
    exponent = tl.full((), 0, tl.int32)
    high = tl.full((), 0, tl.uint32)
    low = tl.full((), 0, tl.uint32)
    token = tl.full((), V - 1, tl.int32)
    index = 0
    found = tl.full((), False, tl.int1)
    while (index < V) & (found == 0):
        bits = tl.load(Values + row * V + index).to(tl.uint32, bitcast=True)
        exponent, high, low = _add32_to64(exponent, high, low, bits)
        larger = (exponent > target_exp) | (exponent == target_exp) & (
            (high > target_high) | (high == target_high) & (low != 0)
        )
        found = (
            (threshold == threshold)
            & (threshold != float("inf"))
            & ((threshold < 0) | (high != 0) & ((threshold == 0) | larger))
        )
