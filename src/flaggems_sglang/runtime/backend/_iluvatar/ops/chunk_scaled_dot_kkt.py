...
def _run(k, beta, g_cumsum, chunk_size, use_trans, warps, stages):
    ...
    if use_g:
        ...
        sg = (0, 0, 0)
        g_ptr = k
    sk, sb, so = (k.stride(), beta_p.stride(), out.stride())
    ieee = k.dtype == torch.float32
    bk = _block_k(K)
    _storeopt_perm_kernel[NT * B * Hg,](
        k,
        beta_p,
        g_ptr,
        out,
...
def chunk_scaled_dot_kkt(k, beta, g_cumsum=None, chunk_size=64):
    return _run(
        k, beta, g_cumsum, chunk_size, use_trans=False, warps=8, stages=4
    )
