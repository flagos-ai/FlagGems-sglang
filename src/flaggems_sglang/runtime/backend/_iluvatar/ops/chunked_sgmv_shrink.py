...
def split4_m64n32k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 4, 64, 32, 128)


def split8_m64n32k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 8, 64, 32, 128)


def split16_m64n32k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 16, 64, 32, 128)


def split8_m64n32k256(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 8, 64, 32, 256)


def split8_m64n32k512(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 8, 64, 32, 512)
...
def split4_m128n64k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 4, 128, 64, 64)


def split6_m128n64k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 6, 128, 64, 64)


def split6_m64n32k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 6, 64, 32, 64)


def split4_m128n64k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 4, 128, 64, 128)


def split8_m128n64k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 8, 128, 64, 128)


def split16_m128n64k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 16, 128, 64, 128)


def split8_m128n64k256(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 8, 128, 64, 256)
...
def chunked_sgmv_shrink(x, weights, batch_info, num_slices=1):
    ...
    if batch_info.permutation is not None:
        ...
        if (
            x.shape[1] >= 1024
            and batch_info.max_len >= 128
            and weights.shape[1] <= 64
        ):
            return _launch_split(
                x, weights, batch_info, stack_num, 4, 64, 64, 128
            )
        split_count = 8 if x.shape[1] >= 1024 else 1
        return _launch_split(
            x, weights, batch_info, stack_num, split_count, 64, 32, 128
        )
    if x.shape[1] >= 1024:
        if batch_info.max_len >= 128 and weights.shape[1] <= 64:
            # BI-V150 benefits from more M tiles and fewer split-K reductions
            # on the long-segment scored case (M=256, N=64, K=4096).
            return _launch_split(
                x, weights, batch_info, stack_num, 4, 64, 64, 128
            )
        if weights.shape[1] <= 64:
            return _launch_split(
                x, weights, batch_info, stack_num, 8, 64, 32, 128
            )
        return _launch_split(x, weights, batch_info, stack_num, 4, 64, 32, 64)
    return _launch(x, weights, batch_info, stack_num, 16, 32, 32, 4, 2)
