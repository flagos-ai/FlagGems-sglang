...
def _gemm(
    ...
    for step in range(triton.cdiv(K, BK * SPLITS)):
        kk = k + step * BK * SPLITS
        ap = A + rows[:, None] * A0 + kk[None, :] * A1
        bp = B + kk[:, None] * B0 + n[None, :] * B1
        if K % (BK * SPLITS) == 0:
            a = tl.load(ap)
            if N % BN == 0:
                b = tl.load(bp)
            else:
                b = tl.load(bp, n[None, :] < N, 0)
        else:
            a = tl.load(ap, kk[None, :] < K, 0)
            if N % BN == 0:
                b = tl.load(bp, kk[:, None] < K, 0)
            else:
                b = tl.load(bp, (kk[:, None] < K) & (n[None, :] < N), 0)
...
def _gemm_long(
    ...
    k = tl.arange(0, BK) + split * BK
    acc = tl.zeros((BM, BN), tl.float32)
    for step in tl.static_range((K + BK * SPLITS - 1) // (BK * SPLITS)):
        kk = k + step * BK * SPLITS
        ap = A + rows[:, None] * A0 + kk[None, :] * A1
        bp = B + kk[:, None] * B0 + n[None, :] * B1
        if K % (BK * SPLITS) == 0:
