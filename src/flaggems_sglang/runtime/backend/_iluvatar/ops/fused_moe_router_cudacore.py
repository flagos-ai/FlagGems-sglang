...
def _logits_kernel(
    ...
    tl.store(logits_ptr + t1 * E + offs_e, acc1, mask=e_live & (t1 < BS))
    tl.store(logits_ptr + t2 * E + offs_e, acc2, mask=e_live & (t2 < BS))
    tl.store(logits_ptr + t3 * E + offs_e, acc3, mask=e_live & (t3 < BS))
