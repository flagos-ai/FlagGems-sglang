...
def _launch_serial(
    x, weights, batch_info, stack_num, round_mode=0, block_m=8, block_n=16
):
    token_count, hidden_size = x.shape
    output_size = weights.shape[1]
    max_segment_length = max(1, batch_info.max_len)
...
def _launch_split_permuted(
    ...
    _scatter_permuted_kernel[(triton.cdiv(output_elements, 8192),)](
        packed_output,
        batch_info.permutation,
        output,
        output_elements,
...
def _launch_direct_permuted(x, weights, batch_info, stack_num):
    ...
    packed_x = torch.empty_like(x)
    input_elements = token_count * hidden_size
    _gather_permuted_kernel[(triton.cdiv(input_elements, 8192),)](
        x,
        batch_info.permutation,
        packed_x,
        input_elements,
    ...
    is_short_n16 = batch_info.max_len <= 8 and output_size <= 16
    block_m = 16 if is_short_n16 else 64 if batch_info.max_len <= 64 else 128
    block_n = (
        16
        if is_short_n16
        else 32 if output_size <= 32 and hidden_size <= 1024 else 64
    )
...
def chunked_sgmv_shrink(x, weights, batch_info, num_slices=1):
    ...
    if x.dtype == torch.float32:
        return _launch_serial(x, weights, batch_info, stack_num, 0, 16, 32)
    ...
    if batch_info.permutation is not None:
        return _launch_direct_permuted(x, weights, batch_info, stack_num)
    if x.shape[1] >= 1024:
        return _launch(
            x,
            weights,
