...
def _sgemm_lora_a_split_kernel(
    ...
    partial_offsets = (
        ...
        + positions[:, None] * stride_partial_row
...
def _sgemm_lora_a_reduce_kernel(
    ...
    covered_tokens = tl.load(seg_indptr_ptr + batch_size)
    mask = (rows[:, None] < covered_tokens) & (
        offsets_n[None, :] < output_size
    )
    accumulator = tl.zeros((block_m, block_n), dtype=tl.float32)
    ...
    if has_permutation:
        output_rows = tl.load(
            permutation_ptr + rows, mask=rows < covered_tokens, other=0
        )
    else:
        output_rows = rows
    output_offsets = (
        output_rows[:, None] * stride_output_row
        + offsets_n[None, :] * stride_output_rank
    )
...
def _launch_split(
    ...
    split_arguments = (
        ...
        batch_info.seg_indptr,
        batch_info.weight_indices,
        x if batch_info.permutation is None else batch_info.permutation,
        partial,
        hidden_size,
        output_size,
        max_segment_length,
...
def tune_s8_m64n32k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 8, 64, 32, 128)


def tune_s12_m64n32k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 12, 64, 32, 64)


def tune_s12_m64n32k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 12, 64, 32, 128)


def tune_s20_m64n32k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 20, 64, 32, 64)


def tune_s24_m64n32k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 24, 64, 32, 64)


def tune_s4_m64n64k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 4, 64, 64, 64)


def tune_s6_m64n64k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 6, 64, 64, 64)


def tune_s8_m64n64k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 8, 64, 64, 64)


def tune_s10_m64n64k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 10, 64, 64, 64)


def tune_s12_m64n64k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 12, 64, 64, 64)


def tune_s16_m64n64k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 16, 64, 64, 64)


def tune_s8_m64n64k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 8, 64, 64, 128)


def tune_s4_m64n64k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 4, 64, 64, 128)


def tune_s6_m64n64k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 6, 64, 64, 128)


def tune_s10_m64n64k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 10, 64, 64, 128)


def tune_s8_m32n64k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 8, 32, 64, 128)


def tune_s12_m64n32k128(x, weights, batch_info, stack_num=1):  # noqa: F811
    return _launch_split(x, weights, batch_info, stack_num, 12, 64, 32, 128)


def tune_s16_m64n32k128(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 16, 64, 32, 128)


def tune_direct_m64n64k128(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_direct_m64n64k64(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_direct_m32n64k256(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_direct_m64n32k256(x, weights, batch_info, stack_num=1):
    return _launch(
        x,
        weights,
        batch_info,
...
def tune_s8_m128n64k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 8, 128, 64, 64)


def tune_s8_m256n64k64(x, weights, batch_info, stack_num=1):
    return _launch_split(x, weights, batch_info, stack_num, 8, 256, 64, 64)
...
def chunked_sgmv_shrink(x, weights, batch_info, num_slices=1):
    ...
    if batch_info.permutation is not None:
        if (
            x.shape[1] >= 1024
            and batch_info.max_len <= 8
            and weights.shape[1] <= 16
        ):
            return _launch_split(
                x, weights, batch_info, stack_num, 16, 16, 16, 128
            )
        if x.shape[1] >= 1024 and batch_info.max_len <= 64:
            return _launch_split(
                x, weights, batch_info, stack_num, 12, 64, 32, 128
            )
        if x.shape[1] >= 1024:
            return _launch_split(
                x, weights, batch_info, stack_num, 8, 64, 64, 128
            )
        return _launch_split(x, weights, batch_info, stack_num, 1, 64, 32, 64)
    if x.shape[1] >= 1024:
