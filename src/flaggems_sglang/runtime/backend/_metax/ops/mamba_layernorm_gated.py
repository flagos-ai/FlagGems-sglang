...
def _performance_2d_kernel(
    ...
    rows = row_block * rows_per_program + tl.arange(0, rows_per_program)
    columns = tl.arange(0, group_size)
    mask = rows[:, None] < row_count
    offsets = rows[:, None] * hidden + group * group_size + columns[None, :]
    feature_offsets = group * group_size + columns
    values = tl.load(x_ptr + offsets, mask=mask, other=0.0).to(tl.float32)
    gate = tl.load(z_ptr + offsets, mask=mask, other=0.0).to(tl.float32)
    weight = tl.load(weight_ptr + feature_offsets).to(tl.float32)
...
@triton.jit
def _performance_2d_mixed_kernel(
    x_ptr,
    weight_ptr,
    bias_ptr,
    z_ptr,
...
def mamba_layernorm_gated(
    ...
    if (
        ...
        if x.dtype == torch.bfloat16:
            if row_count == 1:
                rows_per_program, num_warps = 1, 4
            elif row_count == 8:
                rows_per_program, num_warps = 8, 8
            elif row_count == 64:
                rows_per_program, num_warps = 4, 4
            elif row_count == 4096:
                rows_per_program, num_warps = 32, 4
            else:
                rows_per_program, num_warps = 16, 4
            kernel = (
                _performance_2d_mixed_kernel
                if row_count in (512, 4096)
                else _performance_2d_kernel
            )
            kernel[(group_count, triton.cdiv(row_count, rows_per_program))](
                x,
                weight,
                bias,
                z,
                ...
                rows_per_program,
                num_warps=num_warps,
                num_stages=1 if row_count == 4096 else 2,
            )
            return output
        if row_count in (1, 8, 512, 4096):
            rows_per_program, num_warps = 8, 4
        elif row_count == 64:
            rows_per_program, num_warps = 2, 1
        else:
            rows_per_program, num_warps = 1, 1
        grid_cap = 1048576
