...
@pytest.mark.parametrize("shape, dim", CUMMIN_CASES)
def test_accuracy_cummin(dtype, shape, dim):
    ...
    try:
        ref_v, ref_i = torch.cummin(x, dim=dim)
    except RuntimeError as e:
        pytest.skip(f"torch.cummin does not support {dtype} on CUDA: {e}")

    with flag_dnn.use_dnn():
        out_v, out_i = torch.cummin(x, dim=dim)
...
def test_accuracy_cummin_out_param(dtype):
    ...
    ref_v, ref_i = torch.cummin(x, dim=dim)
    with flag_dnn.use_dnn():
        ret = torch.cummin(x, dim=dim, out=(out_v, out_i))
...
@pytest.mark.cummin
def test_accuracy_cummin_edge_cases():
    ...
    ref_v, ref_i = torch.cummin(x_nan, dim=dim)
    with flag_dnn.use_dnn():
        out_v, out_i = torch.cummin(x_nan, dim=dim)
    ...
    ref_v, ref_i = torch.cummin(x_empty, dim=dim)
    with flag_dnn.use_dnn():
        out_v, out_i = torch.cummin(x_empty, dim=dim)
    torch.testing.assert_close(out_v, ref_v)
    ...
    ref_v, ref_i = torch.cummin(x_non_contig, dim=dim)
    with flag_dnn.use_dnn():
        out_v, out_i = torch.cummin(x_non_contig, dim=dim)
    torch.testing.assert_close(out_v, ref_v)
