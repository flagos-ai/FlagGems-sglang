...
def test_accuracy_ge(dtype, input_shape, other_spec):
    x, y = get_test_inputs(input_shape, other_spec, dtype, flag_dnn.device)

    ref_out = torch.ge(x, y)
...
@pytest.mark.ge
def test_accuracy_ge_with_out_param():
    ...
    torch.ge(x, y, out=ref_out)
    with flag_dnn.use_dnn():
        torch.ge(x, y, out=custom_out)
