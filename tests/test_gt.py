...
def test_accuracy_gt(dtype, input_shape, other_spec):
    x, y = get_test_inputs(input_shape, other_spec, dtype, flag_dnn.device)

    ref_out = torch.gt(x, y)
...
@pytest.mark.gt
def test_accuracy_gt_with_out_param():
    ...
    torch.gt(x, y, out=ref_out)
    with flag_dnn.use_dnn():
        torch.gt(x, y, out=custom_out)
