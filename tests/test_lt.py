...
def test_accuracy_lt(dtype, input_shape, other_spec):
    x, y = get_test_inputs(input_shape, other_spec, dtype, flag_dnn.device)

    ref_out = torch.lt(x, y)
...
@pytest.mark.lt
def test_accuracy_lt_with_out_param():
    ...
    torch.lt(x, y, out=ref_out)
    with flag_dnn.use_dnn():
        torch.lt(x, y, out=custom_out)
