...
def _probe_flaggems():
    try:
        version = flaggems_sglang.__version__
        ENV_INFO["flaggems_sglang"] = {"version": version}
        pinfo(f"flaggems_sglang detected ... {version}")
    except Exception as e:
        perror(f"{e}")
        perror(
            "flaggems_sglang has not been installed, please run `uv pip install -e .`"
        )
        sys.exit(-1)

    try:
        vendor = flaggems_sglang.vendor_name
        ENV_INFO["flaggems_sglang"]["vendor"] = vendor
        pinfo(f"flaggems_sglang vendor detection ... {vendor}")
    except Exception as e:
        perror(f"{e}")
        perror("flaggems_sglang failed to detect vendor info.`")
        sys.exit(-1)

    try:
        device = flaggems_sglang.device
        ENV_INFO["flaggems_sglang"]["device"] = device
        pinfo(f"flaggems_sglang device detection ... {device}")
    except Exception as e:
        perror(f"{e}")
        perror("flaggems_sglang failed to detect device info.`")
        sys.exit(-1)
...
def get_env(gpu_ids):
    env = os.environ.copy()
    vendor = ENV_INFO.get("flaggems_sglang", {}).get("vendor", "")
        ...
        while True:
            ...
            try:
                if op in CFG.accuracy_marks:
                    ...
                    acc = notfound_result
                    display_queue.put(
                        ("done", gpu_id, "accuracy", op, "NotFound", 0)
                    )
                ...
                if op in CFG.benchmark_marks:
                    ...
                    display_queue.put(
                        ("done", gpu_id, "benchmark", op, "NotFound", 0)
                    )

                # Upstream FlagGems calls this get_customized_ops; this repo
                # still uses the older get_current_device_extend_op name.
                backend = flaggems_sglang.runtime.backend
                get_ops = getattr(
                    backend,
                    "get_customized_ops",
                    getattr(backend, "get_current_device_extend_op", None),
                )
                customized_ops = [o[0] for o in get_ops()] if get_ops else []
                result = {
                ...
                display_queue.put(
                    ("done", gpu_id, "benchmark", op, "Error", 0)
                )
...
def display_loop(queue, display, workers):
    ...
    while len(exited) < len(workers):
        ...
        if kind == "exit":
            ...
            n = per_gpu_done.get(gpu_id, 0)
            display.update_gpu(
                gpu_id, f"{DIM}[GPU {gpu_id:2d}] done ({n} ops){NC}"
            )
            ...
            if IS_TTY:
                ...
                display.log(
                    f"[INFO] [{ts}][GPU {gpu_id:2d}]" f" {label} {op_col} ..."
                )
            ...
            if not IS_TTY:
                ...
                log_line += (
                    f"  ({pct:>3}% {ops_done:>{total_w}}/{total_ops} ops)"
                )
...
def collect_marks(ops):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if os.path.exists(acc_file):
            ...
            if accuracy_marks:
                pinfo(
                    f"Found accuracy tests for {len(accuracy_marks)} operators"
                )
            else:
        ...
        if os.path.exists(bench_file):
            ...
            if benchmark_marks:
                pinfo(
                    f"Found benchmark tests for {len(benchmark_marks)} operators"
                )
            else:
                pwarn(
                    "Marks file empty, falling back to all ops for benchmark"
                )
                benchmark_marks = set(ops)
        else:
            pwarn(
                "Failed to collect benchmark marks, all ops will be benchmarked"
            )
            benchmark_marks = set(ops)
...
def main():
    ...
    pinfo(
        f"Test started at ... {test_start_time.strftime('%Y-%m-%d %H:%M:%S')}"
    )
    ...
    for gpu in gpu_ids:
        ...
        p._gpu_id = (
            gpu  # Attach gpu_id for display_loop to identify dead workers
        )
        p.start()
    ...
    total_duration = round(
        (test_end_time - test_start_time).total_seconds(), 2
    )
