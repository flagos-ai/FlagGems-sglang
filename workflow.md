...
op = '<op>'
paths = [Path(f'src/flag_dnn/ops/{op}.py')]
for p in paths:
    if not p.exists():
        continue
    text = p.read_text()
...
| 目的 | 路径 |
| --- | --- |
| 算子实现 | `src/flag_dnn/ops/<op>.py` |
| NVIDIA autotune 配置 | `src/flag_dnn/runtime/backend/_nvidia/tune_configs.yaml` |
| 功能测试 | `tests/test_<op>.py` |
| 性能测试 | `benchmark/test_<op>_perf.py` |
| ops 导出 | `src/flag_dnn/ops/__init__.py` |
| 顶层注册 | `src/flag_dnn/__init__.py` |
...
- 在 `src/flag_dnn/ops/<op>.py` 完成 host 逻辑、kernel launcher 和 Triton kernel。
...
1. `src/flag_dnn/ops/__init__.py` 导出。
2. `src/flag_dnn/__init__.py` 注册。
